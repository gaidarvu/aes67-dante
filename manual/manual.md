...
Dante Controller необходим для настройки и связки Dante-устройств (матрицы, микрофонов, динамиков, мини-ПК и т. д.).
...
### Включение поддержки aes67 на аудиокоммутаторе
...
На скриншоте скоммутированы Dante-устройства на примере проекта Бурназяна.

По вертикали — `Transmitters (OUT)`.

По горизонтали — `Receivers (IN)`.

При настройке устройства (`sources`) демона AES67 оно отобразится в Dante Controller (выделены синим цветом). 

![alt text](pics/image.png)

**Для примера настроим передачу звука для конференции, отправив звук с мини-ПК конференции на динамик операционной и направив звук с микрофонов операционной на ПК конференции.**

Работы по настройке можно разделить на 2 этапа:

1.	Скоммутировать устройства с помощью программы Dante Controller.
2.	Перенаправить звук с помощью программы TIGER D1616N.

**Настройка звука для ПК конференции.**

Данную настройку из 4 пунктов можно выполнить, если ansible не создал source или вы решили удалить текущий source и создать новый.

1.	Подключаемся к `http://10.30.1.35:8080/`.
2.	Во вкладке `Sources` создаём новый «виртуальный выход».
3.	Редактируем поля, отмеченные стрелками на картинке, остальные не трогаем.
4.	RTP-адрес берём из вкладки `General`.

![alt text](<pics/image copy.png>)

После данной настройки нажимаем submit, теперь в программе dante controller в списке Transmitters (OUT) появится устройство CONFERENCE_OR1.
...
Каналы `1` и `2` мини-ПК конференции направлены на матрицу `RX 7`, `RX 8`.
...
**Коммутация звука внутри матрицы** (принцип тот же, что и для аналогового звука)

Выше мы подали звук с мини-ПК конференции на вход матрицы `RX 7`, `RX 8`.

Теперь назначаем любой свободный выход для динамика и коммутируем их между собой.

Для удобства рекомендуется использовать для динамиков выходы `TX 15`, `TX 16`.

![alt text](<pics/image copy 4.png>)

Открываем `Dante Controller` и коммутируем аудиоматрицу с динамиками, как показано на рисунке.

Выходы `TX 15`, `TX 16` матрицы направляем на динамики `SPEAKER-01`, `SPEAKER-02`.

![alt text](<pics/image copy 5.png>)

**Направляем звук с микрофона операционной в мини-ПК конференции**

В Dante Controller коммутируем микрофоны с аудиоматрицей на входы `RX 3`, `RX 4`, как показано на рисунке.

![alt text](<pics/image copy 6.png>)

Далее в программе `TIGER D1616N` необходимо перенаправить звук с `RX3`, `RX4` на TX, которые были выбраны при создании `мультикастов` (см. Настройка мультикастов в Dante Controller) и настройке `sink` (см. Добавление Sink на примере конференции)
